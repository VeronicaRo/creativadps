 ...
 *
 * @param bool $no_check_log If true, do not check sending history. Use for testing.
 ...
 * @since 9.4.3
 */
function crb_plugin_update_notifier( $no_check_log = false, &$result = false, &$info = array() ) {
	...
	//print_r($pl_updates->checked); return;
	...
	foreach ( $updates->response as $plugin => $new_data ) {
		if ( ! $no_check_log && isset( $history[ $plugin ][ $new_data->new_version ] ) ) {
			continue;
		}
		...
		$result = cerber_send_notification( 'generic', array(
