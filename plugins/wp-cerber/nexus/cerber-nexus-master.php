...
require_once( __DIR__ . '/cerber-slave-list.php' );
...
final class CRB_Nexus {
	...
	static function send( $request, $slave_id = null ) {
		...
		nexus_update_slave( $website->id, array( 'last_http' => $nexus_last_http ) );
		$nexus_slave_id = $website->id;
		$nexus_slave_name = $website->site_name;
		...
		if ( ! crb_is_wp_error( $network ) ) {
            ...
            <div style="padding: 4em;">
				...
				<?php
				$website = nexus_get_context();
				if ( $website ) {
					echo '<p><a href="' . nexus_get_back_link() . '">Switch back to the main website</a></p>';
					echo '<p><a href="' . $website->site_url . '/wp-admin/admin.php?page=cerber-nexus" target="_blank">Check the access settings on ' . $website->site_name . '</a></p>';
				}
				?>
	...
	static function net_send_request( $payload, $slave ) {
		...
		if ( empty( $slave->site_pass )
			...
			return new WP_Error( 'invalid_slave', 'Mandatory fields for connecting to the managed website #' . $slave->id . ' are invalid or missing' );
		}
		...
		nexus_diag_log( 'Website "' . $slave->site_name . '" has generated response for ' . $ret['p_time'] );
