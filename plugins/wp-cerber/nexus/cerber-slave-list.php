...
class CRB_Slave_Table extends WP_List_Table {
	...
	function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'plugin_v':
				if ( $val ) {
					$ret = '<span class="crb-slave-ver" title="' . cerber_auto_date( $item['refreshed'], false ) . '">' . $val . '</span>';
	...
	function no_items() {
		if ( ! empty( $_GET['s'] ) ) {
			...
			echo __( 'No websites configured.', 'wp-cerber' ) . ' <a class="thickbox" href="' . CRB_ADD_SLAVE_LNK . '">' . __( 'Add a new one', 'wp-cerber' ) . '</a> | <a href="' . $no_master . '">' . __( 'Disable main website mode', 'wp-cerber' ) . '</a>';
		}
	}
}