 ...
 * @since 8.9.6.1
 */
function cerber_send_notification( $type, $msg_parts = array(), $channels = array(), $ignore = false, $args = array() ) {
	...
	if ( ( 1 > $mf )
	     && ! in_array( $type, array( 'shutdown', 'generic' ) )
	...
	$footer .= "\n\n\n" . __( 'This message was created by', 'wp-cerber' ) . ' WP Cerber Security ' . ( lab_lab() ? 'PRO ' : '' ) . ( 1 > $mf ? CERBER_VER : '' );
...
function crb_send_lockout( $ip_address = '' ) {
	...
	if ( array_filter( $channels ) ) {
		return cerber_send_notification( 'lockout', array( 'ip' => $ip_address ), $channels );
	}
...
function cerber_do_hourly_2() {
	...
	if ( crb_get_settings( 'monthly_report' )
        ...
        if ( date( 'j', $now ) == $send_day
			...
			$result = cerber_send_notification( 'report', array(), array(), false, array( 'report_id' => 'one_month' ) );
