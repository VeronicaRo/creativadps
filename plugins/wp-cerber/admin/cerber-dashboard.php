			...
			switch ( $do ) {
				...
				case 'testnotify':
					...
					if ( $test_type == 'plugin_updates' ) {
						if ( false === crb_plugin_update_notifier( true, $sent, $msg ) ) {
							$sent = false;
						}
					}
					else {
						...
						$sent = cerber_send_notification( $test_type, array( 'subj' => ' *** ' . __( 'TEST MESSAGE', 'wp-cerber' ) . ' *** ' ), $channels, true, array( 'report_id' => $period ) );
...
function cerber_admin_footer() {
	...
	if ( $uid ) {
		...
		if ( ( $user = get_userdata( $uid ) )
		     && crb_is_username_prohibited( $user->user_login ) ) {
			?>
            <script>
